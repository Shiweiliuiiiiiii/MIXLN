...
def parse_args(args):
    ...
    parser.add_argument("--save_every", type=int, default=50000000)
...
def main(args):
    ...
    wandb.login(key='ecf16fa8d6978dcb68a0f6b3fda0c9ef99dab756')
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if not layer_wise_flag:
            ...
            #     # 计算每一层的梯度范数之和（在 optimizer.step() 之前）
            #     layer_grad_norms = {}
            ...
            #                 if token == 'layers' and i + 1 < len(tokens):
            #                     layer_number = tokens[i + 1]
            #                     break
            #             if layer_number is not None:
            #                 if layer_number not in layer_grad_norms:
            #                     layer_grad_norms[layer_number] = 0.0
            #                 layer_grad_norms[layer_number] += grad_norm

            #     # 准备绘图数据
            #     # 将 layer_number 转换为整数并排序
            #     sorted_layers = sorted(layer_grad_norms.keys(), key=lambda x: int(x))
            #     x = [int(layer) for layer in sorted_layers]
            #     y = [layer_grad_norms[layer] for layer in sorted_layers]

            #     print ("layer_grad_norms",layer_grad_norms)
            ...
            #     plt.title(f'Sum of Gradient Norms per Layer at Step {update_step}')
            #     plt.tight_layout()
            #     # 保存图像
            #     model_name="model_name"
            #     if "130" in args.model_config:
            #         model_name = "130M"
            #     if "250" in args.model_config:
            #         model_name = "250M"
            #     plt.savefig(f'{model_name}_gradient_norms_step_{update_step}.png')
            #     plt.close()
